...
class Room(Canvas):
    ...
    def __init__(self,master):
        ...
        self.bind("<ButtonPress-3>",self.rotate)
        self.moving = Furniture("square",750,750,.1,.1,10,10)
        self.movable = 1
    ...
    def rotate(self,event):
        ...
        if(self.moving.shape == "rectangle" or self.moving.shape == "oval"):
            x = self.moving.length
            self.moving.length = self.moving.width
            self.moving.width = x
            if(DEBUG):
                print ("{},{}").format(self.moving.width,self.moving.length)
            self.update()
    ...
    def update(self):
        ...
        for i in range(len(self.furniture)):
            ...
            if(self.furniture[i].shape == "oval"):
                self.create_oval(self.furniture[i].x,self.furniture[i].y,self.furniture[i].x+self.furniture[i].length,self.furniture[i].y+self.furniture[i].width,fill = "black")
    ...
    def drop(self,event):
        ...
        for i in range(len(self.furniture)):
            if(self.moving != self.furniture[i]):
                if(self.furniture[i].x<self.moving.x<self.furniture[i].x+self.furniture[i].width or self.furniture[i].x < self.moving.x + self.moving.y < self.furniture[i].x +self.furniture[i].width):
                        if(self.furniture[i].y <self.moving.y<self.furniture[i].y+self.furniture[i].length or self.furniture[i].y < self.moving.y + self.moving.length<self.furniture[i].y + self.furniture[i].length):
                            #self.moving.x = self.movingx
                            #self.moving.y = self.movingy
                            if(self.furniture[i].x+self.furniture[i].width/2 > self.moving.x + self.moving.width):
                                self.moving.x = self.furniture[i].x -self.moving.width
                                if(self.moving.x + self.moving.width > self.furniture[i].x):
                                    self.moving.x = self.furniture[i].x + self.furniture[i].width
                            else:
                                self.moving.x = self.furniture[i].x + self.furniture[i].width
                                if(self.moving.x < self.furniture[i].x + self.furniture[i].width):
                                    self.moving.x = self.furniture[i].x - self.moving.width
        self.update()
...
class Menu(Frame):
    ...
    def scan_x(self):
        self.x=int(self.getDistance())
        if (DEBUG):
            print self.x

    def scan_y(self):
        self.y=int(self.getDistance())
        if (DEBUG):
            print self.y
        
    def convert(self):
        try:
            Menu.l3=Label(self.master,text=str(int(float(Menu.e3.get())*30.48)))
    ...
    def furnish(self):
        if  ((self.x!=None) and (self.x!=None) and (Menu.SCREEN_Y>=self.x) and (Menu.SCREEN_Y>=self.y)):
            ...
            if (Menu.C.get()):
                Menu.room.createFurniture("oval",Menu.room.winfo_width(),Menu.room.winfo_height(),int(Menu.s*y),int(Menu.s*x),100,100)
            else:
                Menu.room.createFurniture("rectangle",Menu.room.winfo_width(),Menu.room.winfo_height(),int(Menu.s*y),int(Menu.s*x),100,100)
            self.x = None
            self.y = None

        else:
            if (((str.isdigit(Menu.e1.get()))) and (str.isdigit(Menu.e2.get())) and (Menu.room.winfo_width()>=int(Menu.e1.get())) and (Menu.room.winfo_width()>=int(Menu.e2.get()))):
                if (int(Menu.e1.get())>int(Menu.e2.get())):
                    x=int(Menu.e1.get())
                    y=int(Menu.e2.get())
                else:
